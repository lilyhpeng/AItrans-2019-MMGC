...
class Agent():
  ...
  def act(self, state_np):
    ...
    # print("act input state size", state.size())
    state_torch = state.unsqueeze(0).cuda()  # added by penghuan on 20190527
    with torch.no_grad():
      return (self.online_net(state_torch) * self.support).sum(2).argmax(1).item()  # changed by penghuan on 20190527
