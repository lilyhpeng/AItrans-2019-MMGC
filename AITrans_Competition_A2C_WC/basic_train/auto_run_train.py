...
def test(nn_model):
    ...
    video_trace_prefix = './video_trace/' + VIDEO_TRACE + '/frame_trace_'
    ...
    # past_info setting
    past_frame_num = 7500
    S_time_interval = deque([0] * past_frame_num)
    S_send_data_size = deque([0] * past_frame_num)
    S_chunk_len = deque([0] * past_frame_num)
    S_rebuf = deque([0] * past_frame_num)
    S_buffer_size = deque([0] * past_frame_num)
    S_end_delay = deque([0] * past_frame_num)
    S_chunk_size = deque([0] * past_frame_num)
    S_play_time_len = deque([0] * past_frame_num)
    S_decision_flag = deque([0] * past_frame_num)
    S_buffer_flag = deque([0] * past_frame_num)
    S_cdn_flag = deque([0] * past_frame_num)
    S_skip_time = deque([0] * past_frame_num)
    # params setting
    call_time_sum = 0
    while True:
        ...
        S_time_interval.popleft()
        S_send_data_size.popleft()
        S_chunk_len.popleft()
        S_buffer_size.popleft()
        S_rebuf.popleft()
        S_end_delay.popleft()
        S_play_time_len.popleft()
        S_decision_flag.popleft()
        S_buffer_flag.popleft()
        S_cdn_flag.popleft()
        S_skip_time.popleft()
...
for turn in range(start_turn, end_turn):
    nn_model = './model/train/nn_model_ep_' + str(turn) + '.ckpt'
    reward, runtime = test(nn_model)
    max_reward = max(max_reward, reward)
    with open(result_path, 'a', encoding='utf-8') as f:
        info = str(turn) + ',' + ('%.2f' % reward) + '\n'
        f.write(info)
    print('turn %d:' % turn, reward, 'now_max:', max_reward)
