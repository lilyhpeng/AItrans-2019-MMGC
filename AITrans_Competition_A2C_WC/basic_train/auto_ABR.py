...
# NN_MODEL = "/root/mmgc/team/The World/submit/results/nn_model_ep_72.ckpt"
# NN_MODEL = './model/train1/nn_model_ep_.ckpt'
S_DIM = 16
A_DIM = 8
...
def get_time(S_skip_time, S_rebuf, S_decision_flag):
    skip_time_sum = S_skip_time[-1]
    rebuf_time_sum = S_rebuf[-1]
    n = 7499
    for i in range(n-1, -1, -1):
        if S_decision_flag[i]: break
        skip_time_sum += S_skip_time[i]
        rebuf_time_sum += S_rebuf[i]
    return skip_time_sum, rebuf_time_sum
...
class Algorithm:
    ...
    def run(self, time, S_time_interval, S_send_data_size, S_chunk_len, S_rebuf, S_buffer_size, S_play_time_len,
        ...
        skip_time_sum, rebuf_time_sum = get_time(S_skip_time, S_rebuf, S_decision_flag)
        ...
        state[13] = skip_time_sum / 2.0
        state[14] = rebuf_time_sum / 10.0
        state[15] = bit_rate / 5.0
        ...
        return bit_rate, target_buffer, 2.0
