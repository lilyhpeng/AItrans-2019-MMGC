...
DEFAULT_TARGET_BUFFER = 0  # default target buffer,to get smaller initial delay,set it to 0,modified
DEFAULT_LATENCY_LIMIT = 2  # set 3.0 as default latency_limit,added on 20190509
...
SKIP_PENALTY = 0.5
# path setting:
network_trace_dir = '../dataset/network_trace/final_train/'
video_trace_prefix = '../dataset/video_trace/AsianCup_China_Uzbekistan/frame_trace_'
...
SUMMARY_DIR = '../results/summaryAndModel_20190520_v4'   # summary info(tensorboard to view) and model
# others:
NN_MODEL = "../results/summaryAndModel_20190516_v4/nn_model_ep_66000.ckpt"#None
DEBUG = False
RANDOM_SEED = 42
random_seed = 2
video_count = 0
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess:
        ...
        sess.run(tf.global_variables_initializer())  # use gpu
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            if decision_flag:
                ...
                state[8, -1] = np.sum(S_skip_time)  # skip time of last gop
                ...
                if end_of_video:
                    ...
                    action_vec = np.zeros(A_DIM)
                    action_vec[action_num] = 1
                    s_batch.append(np.zeros((S_INFO, S_LEN)))  # initial state
                    a_batch.append(action_vec)  # default action
                else:
                    action_vec = np.zeros(A_DIM)
                    action_num = getActionNum(bit_rate, target_buffer, latency_limit)  # changed on 20190509
                    action_vec[action_num] = 1
                    a_batch.append(action_vec)  # record the action at current state
            ...
            # and set training-related record info to default value: (we abandon the last frames)
