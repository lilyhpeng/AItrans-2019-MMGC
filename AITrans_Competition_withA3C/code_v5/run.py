...
def test(user_id):
    ...
    LOG_FILE_PATH = '../results/AiTransLog/'
    ...
    buffer_record = [0]
    throughput_record = [0]
    throughput_predict_record = [0]
    decision_cnt = 0
    decision_time_record = [0]
    target_buffer_record = [0]
    thr_predict = 0
    ...
    while True:
        ...
        if DRAW:
            if time_interval != 0:
                ...
                target_buffer_record.append(target_buffer)
                # plot throughput
                trace_idx = net_env.get_trace_id()
                throughput_record.append(all_cooked_bw[trace_idx][int(idx / 0.5)] * 1000)
                throughput_predict_record.append(thr_predict)
                # decision_time_record.append(decision_cnt)
                # throughput_abr.append(send_data_size / time_interval / 1000)
        ...
        if not cdn_flag:
            ...
            reward_quality += frame_time_len * float(BIT_RATE[bit_rate]) / 1000
            reward_rebuf += REBUF_PENALTY * rebuf
            reward_latency += LANTENCY_PENALTY * end_delay
            reward_skip += SKIP_PENALTY * skip_frame_time_len
        else:
        ...
        if decision_flag or end_of_video:
            ...
            reward_switch += SMOOTH_PENALTY * (abs(BIT_RATE[bit_rate] - BIT_RATE[last_bit_rate]) / 1000)
            # last_bit_rate
            ...
            if DRAW:
                ...
                plt.ylabel("predict_throughput")
                plt.ylim(0, 6000)
                plt.plot(id_list, throughput_predict_record, '-g')
                ...
                # ax = fig.add_subplot(415)
                # plt.ylabel("throughput_predict")
                # plt.ylim(0, 6000)
                # plt.plot(id_list, throughput_predict_record, '-g')

                # ax = fig.add_subplot(414)
                # plt.ylabel("throughput_calculate")
                # plt.ylim(0, 8000)
                # plt.plot(s_time, throughput_abr, '-g')
            ...
            bit_rate, target_buffer, latency_limit, thr_predict = abr.run(time,
                    S_time_interval,
                    S_send_data_size,
                    S_chunk_len,
                    S_rebuf,
            ...
            # thr_predict = throughput_predict
            # print(thr_predict)
            # -------------------- End --------------------------------
        # plot related code:
        # if DRAW:
        ...
        if end_of_video:
            ...
            #print(reward_quality, reward_rebuf, reward_latency, reward_skip, reward_switch)
            #print('')
            # print(cnt)
