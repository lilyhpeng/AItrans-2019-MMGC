...
S_INFO = 8  # last 8 gop's bit_rate, buffer_size, last gop's download time, next_chunk_size, bandwidth_measurement(throughput and time), delay, fft
...
SUMMARY_DIR = './results/summaryAndModel'   # summary info(tensorboard to view) and model
# others:
NN_MODEL = None
DEBUG = False
DRAW = False
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            if decision_flag:
                ...
                state[2, -1] = discount_thr[-1] / 5
                state[3, -1] = float(sum(S_time_interval)) / 10  # 100 sec,last gop's download time
                state[4, :4] = np.array(next_video_chunk_sizes) / float(np.max(next_video_chunk_sizes)) # mega byte,next gop's size
                state[5, -1] = end_delay  # end-to-end delay
