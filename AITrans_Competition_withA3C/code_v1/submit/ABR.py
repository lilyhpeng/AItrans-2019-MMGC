...
#NN_MODEL = "./results/nn_model.ckpt" # model path settings
...
class Algorithm:
    def __init__(self):
        ...
        self.throughput_record = [0]*5
        self.throughput_record2 = [0] * record_len
        self.call_cnt = 0
        self.variance = 0
        self.IntialVars = []
    ...
    def run(self, time, S_time_interval, S_send_data_size, S_chunk_len, S_rebuf, S_buffer_size, S_play_time_len, S_end_delay,
         ...
         # initialize/reset the state info when begin downloading a video:

         # dequeue history record
         self.state = np.roll(self.state, -1, axis=1)

         # predict throughput
         k = len(S_send_data_size)
         if k >=1 and S_send_data_size[-1] and S_time_interval[-1]:
             self.throughput_record.pop(0)
             self.throughput_record.append(float(S_send_data_size[-1]) / (S_time_interval[-1]) / M_IN_K / M_IN_K)
             self.throughput_record2.pop(0)
             self.throughput_record2.append(float(S_send_data_size[-1]) / (S_time_interval[-1]) / M_IN_K / M_IN_K)
             self.call_cnt += 1
         elif k >= 2 and S_send_data_size[-2] and S_time_interval[-2]:
             self.throughput_record.pop(0)
             self.throughput_record.append(float(S_send_data_size[-2]) / (S_time_interval[-2]) / M_IN_K / M_IN_K)
             self.throughput_record2.pop(0)
             self.throughput_record2.append(float(S_send_data_size[-2]) / (S_time_interval[-2]) / M_IN_K / M_IN_K)
             self.call_cnt += 1
         ...
         if self.call_cnt + 1 > record_len:
             self.variance = np.var(self.throughput_record2)
             bit_rate_average = np.sum(self.throughput_record2)/len(self.throughput_record2)
             bit_rate_median = np.median(self.throughput_record2)
             # if self.variance > 0.15:
             # print(self.variance, bit_rate_average)
         #--------------------------
         ...
         # self.state[0, -1] = S_real_quality[-1] / float(np.max(VIDEO_BIT_RATE))
         self.state[1, -1] = (S_buffer_size[-1] - 2.5) / 2.5  # current buffer size
         self.state[2, -1] = throughput_predict
         ...
         # if end_of_video:
         #     self.if_begin = True
         # print(bit_rate, target_buffer, self.latency_limit)
