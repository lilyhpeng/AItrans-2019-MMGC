...
S_INFO = 9  # last 8 gop's bit_rate, buffer_size, last gop's download time, next_chunk_size, bandwidth_measurement(throughput and time), delay, fft
...
SUMMARY_DIR = '../results/summaryAndModel_20190605_tmp'   # summary info(tensorboard to view) and model
# others:
NN_MODEL = None#"../results/summaryAndModel_20190527_high/nn_model_ep_0001.ckpt"
DEBUG = False
DRAW = False
RANDOM_SEED = 42
random_seed = 2
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            if decision_flag:
                ...
                thr_var = np.var(throughput_record)
                # ##################################################################################################
                ...
                state[2, -1] = thr_predict / 5
                state[3, -1] = sum(S_rebuf) / 10
