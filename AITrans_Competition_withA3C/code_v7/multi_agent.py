...
# modified by penghuan on 20181202:
# S_INFO = 6  # bit_rate, buffer_size, last gop's download time, next_chunk_size, bandwidth_measurement(throughput and time), delay
S_INFO = 10  # last 8 gop's bit_rate, buffer_size, last gop's download time, next_chunk_size, bandwidth_measurement(throughput and time), delay, fft
S_LEN = 10  # take how many frames in the past
A_DIM = 4  # modified by penghuan on 20181127
A_BIT_DIM = 4
...
SUMMARY_DIR = '../results/summaryAndModel_20190608_all'   # summary info(tensorboard to view) and model
# others:
NN_MODEL = None
DEBUG = False
DRAW = False
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        action_num = getActionNum(bit_rate, target_buffer)
        ...
        throughput_record = [0] * thr_record_len
        throughput_predict = 0
        thr_record_cnt = 0
        latency_limit = 2.0
        ...
        while True:  # experience video streaming forever
            ...
            decision_flag, buffer_flag, cdn_flag, skip_flag, end_of_video = net_env.get_video_frame(bit_rate, target_buffer, latency_limit)
            ...
            if decision_flag:
                ...
                #                     throughput_record[-3] + 0.15 * throughput_record[-2] + 0.5 * throughput_record[-1]
                ...
                # ################################# bandwidth predict algorithm ####################################
                # if S_send_data_size[-1] and S_time_interval[-1]:
                #     throughput_record.pop(0)
                #     throughput_record.append(
                #         float(S_send_data_size[-1]) / (S_time_interval[-1]) / M_IN_K / M_IN_K)
                # elif S_send_data_size[-2] and S_time_interval[-2]:
                #     throughput_record.pop(0)
                #     throughput_record.append(
                #         float(S_send_data_size[-2]) / (S_time_interval[-2]) / M_IN_K / M_IN_K)
                # cnt += 1
                # if cnt < 5:
                #     thr_predict = sum(throughput_record[-cnt:]) / cnt
                # else:
                #     thr_predict = 0.15 * throughput_record[-5] + 0.15 * throughput_record[-4] + \
                #                   0.15 * throughput_record[-3] + 0.15 * throughput_record[-2] + \
                #                   0.6 * throughput_record[-1]
                tmp = 0
                for i in reversed(range(1, len(S_send_data_size)+1)):
                    thr = 0
                    if S_time_interval[-i] > 0:
                        thr = S_send_data_size[-i] / S_time_interval[-i] / M_IN_K / M_IN_K
                    if (thr-tmp > 0.000001) and(thr > 0.000001):
                        throughput_record.pop(0)
                        throughput_record.append(thr)
                        thr_record_cnt += 1
                thr_cnt = min(thr_record_cnt, thr_record_len)
                thr_var = np.var(throughput_record[-thr_cnt:])
                # ##################################################################################################

                # print(state.shape)
                # this should be S_INFO number of terms
                state[0, -1] = VIDEO_BIT_RATE[bit_rate] / float(np.max(VIDEO_BIT_RATE))  # last 8 gop's quality
                state[1, -1] = (buffer_size - 2.5) / 2.5  # current buffer size
                state[2, -1] = S_send_data_size[-1] / S_time_interval[-1] / M_IN_K / M_IN_K / 5  # last gop's thr
                state[3, -1] = sum(S_rebuf) / 5
                state[4, :4] = np.array(next_video_chunk_sizes) / float(np.max(next_video_chunk_sizes)) # mega byte,next gop's size
                state[5, -1] = (cdn_newest_id - download_id) / 25
                state[6, -1] = sum(S_skip_time) / 5
                state[7, -1] = sum(S_send_data_size) / M_IN_K / M_IN_K
                state[8, -1] = thr_var
                state[9, -1] = sum(S_send_data_size) / gop_len / M_IN_K / float(np.max(VIDEO_BIT_RATE))
                # # added by penghuan on 20181202:add fft feature of last 8 gop's bitrate
                # _fft = np.fft.fft(state[0])
                # state[6] = _fft.real
                # state[7] = _fft.imag
                # quality_fft = []
                # for i in range(0, len(_fft.real)):
                #     quality_fft.append(np.sqrt(_fft.real[i], _fft.imag[i]))
                # state[6] = quality_fft
                ...
                # bit_rate, target_buffer = getBitrateAndTargetbuffer(action_num)

                # decide target_buffer and latency_limit:
                if S_buffer_size[-1] < 0.3:
                    target_buffer = 0
                elif S_buffer_size[-1] < 0.5:
                    target_buffer = 1
                elif S_buffer_size[-1] < 1.0:
                    target_buffer = 0
                elif S_buffer_size[-1] < 2.0:
                    target_buffer = 0
                else:
                    target_buffer = 0

                latency_limit = VIDEO_BIT_RATE[bit_rate]/M_IN_K + 0.5
...
# added by penghuan on 20181127:
def getActionNum(bit_rate, target_buffer):
