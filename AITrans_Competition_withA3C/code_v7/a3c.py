...
def compute_gradients(ENTROPY_WEIGHT, s_batch, a_batch, r_batch, terminal, actor, critic):
    """
    batch of s, a, r is from samples in a sequence
    the format is in np.array([batch_size, s/a/r_dim])
    terminal is True when sequence ends as a terminal state
    """
    #print('s_batch.shape[0]', s_batch.shape[0], 'r_batch.shape[0]', r_batch.shape[0], 'a_batch.shape[0]', a_batch.shape[0])
    assert s_batch.shape[0] == a_batch.shape[0]
    assert s_batch.shape[0] == r_batch.shape[0]
    ba_size = s_batch.shape[0]

    # current state value(100 steps)(predicted value):
    ...
    # calculate the discounted reward of every step(real value):
    for t in reversed(range(ba_size - 1)):
        R_batch[t, 0] = r_batch[t] + GAMMA * R_batch[t + 1, 0]

    # the gap between real value and predicted value:
    td_batch = R_batch - v_batch

    actor_gradients = actor.get_gradients(ENTROPY_WEIGHT, s_batch, a_batch, td_batch)
