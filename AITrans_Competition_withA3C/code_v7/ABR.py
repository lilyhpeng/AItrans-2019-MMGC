...
NN_MODEL = "../results/summaryAndModel_20190608_all/nn_model_ep_152000.ckpt"  # model path settings
...
class Algorithm:
    ...
    def run(self, time, S_time_interval, S_send_data_size, S_chunk_len, S_rebuf, S_buffer_size, S_play_time_len,
        ...
        if self.call_cnt < record_len:
            thr_var = np.var(self.throughput_record[-self.call_cnt:])
        ...
        # find how many intervals of last gop's download:
        des = 100
        for i in range(2, 200):
            if S_decision_flag[-i]:
                des = i
                break
        rebuf_time = sum(S_rebuf[-des+1:])
        skip_time = sum(S_skip_time[-des+1:])
        last_gop_size = sum(S_send_data_size) / M_IN_K / M_IN_K

        # get next gop's size/next gop's I frame'size:
        ...
        self.state[2, -1] = self.throughput_record[-1] / 5
        self.state[3, -1] = rebuf_time / 5
        self.state[4, :4] = np.array(next_video_chunk_sizes) / float(np.max(next_video_chunk_sizes))  # next gop's size
        self.state[5, -1] = (cdn_newest_id - download_id) / 25
        self.state[6, -1] = skip_time / 5
        self.state[7, -1] = last_gop_size
        self.state[8, -1] = thr_var
        self.state[9, -1] = last_gop_size / 2 / M_IN_K / float(np.max(VIDEO_BIT_RATE))
...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            split_4 = tflearn.conv_1d(inputs[:, 4:5, :A_BIT_DIM], 128, 4,
            ...
            split_7 = tflearn.conv_1d(inputs[:, 7:8, :], 128, 4, activation='relu')  # FFT of last 8 gop's quality:real
            split_8 = tflearn.conv_1d(inputs[:, 8:9, :], 128, 4, activation='relu')  # FFT of last 8 gop's quality:real
            split_9 = tflearn.conv_1d(inputs[:, 9:10, :], 128, 4, activation='relu')  # FFT of last 8 gop's quality:real
            ...
            merge_net = tflearn.merge([split_0_flat, split_1_flat, split_2_flat, split_3_flat, split_4_flat,
                                       split_5_flat, split_6_flat, split_7_flat, split_8_flat, split_9_flat], 'concat')
